...
void K6502_GetReg(struct K6502_REG_T *reg)
{
	...
	reg->Y = Y;
#ifdef IRQ_NMI_SLIM
	reg->IRQ_NMI_State = IRQ_NMI_State; // bit 0: IRQ,   bit 1: NMI
	reg->IRQ_NMI_Wiring= reg->IRQ_NMI_Wiring; // bit 0: IRQ,   bit 1: NMI
#else
	reg->IRQ_State  = IRQ_State;   // The state of the IRQ pin
	reg->IRQ_Wiring = IRQ_Wiring; // Wiring of the IRQ pin
	reg->NMI_State  = NMI_State;  // The state of the NMI pin
	reg->NMI_Wiring = NMI_Wiring;  // Wiring of the NMI pin
#endif
	reg->g_wPassedClocks = g_wPassedClocks;
}
...
void K6502_SetReg(const struct K6502_REG_T *reg)
{
	...
	Y  = reg->Y;
#ifdef IRQ_NMI_SLIM
	IRQ_NMI_State = reg->IRQ_NMI_State; // bit 0: IRQ,   bit 1: NMI
	IRQ_NMI_Wiring= reg->IRQ_NMI_Wiring; // bit 0: IRQ,   bit 1: NMI
#else
	IRQ_State  = reg->IRQ_State;   // The state of the IRQ pin
	IRQ_Wiring = reg->IRQ_Wiring; // Wiring of the IRQ pin
	NMI_State  = reg->NMI_State;  // The state of the NMI pin
	NMI_Wiring = reg->NMI_Wiring;  // Wiring of the NMI pin
#endif
	g_wPassedClocks = reg->g_wPassedClocks;
}
/*===================================================================*/
/*                                                                   */
 ...
 */
#ifdef IRQ_NMI_SLIM
	IRQ_NMI_Wiring = (byNMI_Wiring?(1<<1):0) | (byIRQ_Wiring?(1<<0):0);
#else
...
#ifdef NES_INSTRUCT_TBL_SUPPORT
...
#ifdef IRQ_NMI_SLIM
	if ( ( byD0 & FLAG_I ) && (IRQ_NMI_State ^ IRQ_NMI_Wiring)&IRQ_MASK )
#else
...
#endif // NES_INSTRUCT_TBL_SUPPORT
/*===================================================================*/
/*                                                                   */
/*  K6502_Step() :                                                   */
...
#ifdef IRQ_NMI_SLIM
	if ( (IRQ_NMI_State ^ IRQ_NMI_Wiring) & NMI_MASK)
#else
...
#ifdef IRQ_NMI_SLIM
	if ( (IRQ_NMI_State  ^ IRQ_NMI_Wiring) & IRQ_MASK)
#else
...
// Data
// Absolute,X
static __inline BYTE K6502_ReadAbsX(){ WORD wA0, wA1; wA0 = AA_ABS; wA1 = wA0 + X; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
// Absolute,Y
static __inline BYTE K6502_ReadAbsY(){ WORD wA0, wA1; wA0 = AA_ABS; wA1 = wA0 + Y; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
// (Indirect),Y
static __inline BYTE K6502_ReadIY(){ WORD wA0, wA1; wA0 = K6502_ReadZpW( K6502_Read( PC++ ) ); wA1 = wA0 + Y; CLK( ( wA0 & 0x0100 ) != ( wA1 & 0x0100 ) ); return K6502_Read( wA1 ); };
